<?PHP
	...
	}else
	{
		...
		for ($j=0;$j<$i;$j++){
			$user_name[$input_user_contactsID[$j]]=$input_user_name[$j];
			$user_phonenum[$input_user_contactsID[$j]]=$input_user_phonenum[$j];
			$user_response[$input_user_contactsID[$j]]=$input_user_response[$j];
			$user_contactsID[$input_user_contactsID[$j]]=1;
		}
		for ($j=0;$j<$i;$j++){
			if($user_contactsID[$j]==1)
			{
				echo "<data><contactsID>$input_user_contactsID[$j]</contactsID><name>$user_name[$j]</name><phone>$user_phonenum[$j]</phone><response>$user_response[$j]</response></data>";
			}
		}
	}
	echo "</body>";
