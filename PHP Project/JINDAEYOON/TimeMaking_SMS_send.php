<?PHP
	...
	while(1){
		...
		if ($list==NULL)
			break;
		$input_key = $list[No];		
		$sql = "SELECT * FROM TimeMaking_contactsList WHERE No = '$input_key'";
		...
		while(1)
		{
			...
			else
			{
				...
				$input_cont[$j][$i] = 'http://125.209.196.29/TimeMaking_process.php?phonenum='.$input_user_phonenum[$j][$i].'&key='.$input_key;
				echo "EE";
...
#	$sql = "UPDATE Timeaking SET send = 1 WHERE send = 0" ;
	$result = mysql_query($sql, $connect);
	$num = mysql_num_rows($result);
